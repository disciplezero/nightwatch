...
WaitForElementVisible.prototype.elementNotVisible = function(result, now) {
  ...
  if(this.abortOnFailure)
    return this.fail(result, 'Element <%s> not visible', this.expectedValue, defaultMsg);
  else
    return this.pass(result, 'Element <%s> not visible - but not critical', now - this.startTime);
};
