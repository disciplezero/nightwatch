...
WaitForElementPresent.prototype.elementNotFound = function(result, now) {
  ...
  if(this.abortOnFailure)
    return this.fail(result, 'Element <%s> not found', this.expectedValue, defaultMsg);
  else
    return this.pass(result, 'Element <%s> not found - but not critical', now - this.startTime);
};
